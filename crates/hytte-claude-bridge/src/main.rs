...
//!
//! # It is KEYLESS — and that is a correctness requirement, not laziness
//!
//! The bridge validates **no bearer token at all**. It cannot, and here is the
//! precise reason: `hytte-plugin-pet`'s `brain.rs` resolves its key as
//!
//! ```text
...
//! its *own* token would 401 every single request, forever. Keyless is the only
//! shape that works.
//!
...
//!
//! Loopback-only binding is the other half of that: with no auth, reachability
...
//! **no tool calls**; `temperature` is ignored and `max_tokens` is at best
...
//! | `CLAUDE_BRIDGE_MODEL` | unset | `claude --model`; empty leaves claude's own default |
//! | `CLAUDE_BRIDGE_MODE` | `subscription` | `subscription` (persisted session) or `reprompt` |
//! | `CLAUDE_BRIDGE_TIMEOUT_SECS` | `8` | per-request budget; must stay under the client's 10s |
//! | `CLAUDE_BRIDGE_STATE_DIR` | `$XDG_STATE_HOME/hytte-claude-bridge` | the child's cwd, which is what scopes claude's on-disk sessions |
...
enum Mode {
    ...
    /// only the delta.
    Subscription,
    /// A one-off session per turn, with the bridge holding the transcript.
    Reprompt,
}
...
impl Settings {
    ...
    ///
    /// `idle_timeout` sits one second under the request budget so a stalled
    /// turn is killed and reaped by the driver — yielding a typed
    /// `Error::IdleTimeout` — *before* the outer budget fires. The outer
    /// timeout remains as a backstop for a turn that is streaming but too slow;
    /// that path can leave the child running (`hive-claude` sets no
    /// `kill_on_drop`), which is logged rather than papered over.
...
async fn main() -> ExitCode {
    ...
    // Fail closed before anything else: see envguard's module docs for why this
    ...
    let config = settings.claude_config();
    let backend = match settings.mode {
        Mode::Subscription => Backend::Subscription(Subscription::new(config)),
        Mode::Reprompt => Backend::Reprompt(Reprompt::new(config)),
    };
    ...
    tracing::info!(
        %addr,
        mode = ?settings.mode,
        model = %if settings.model.is_empty() { "<claude default>" } else { &settings.model },
        budget_s = settings.budget.as_secs(),
        state_dir = %settings.state_dir.display(),
        "hytte-claude-bridge listening (keyless by design; loopback only)",
    );
...
#[cfg(test)]
mod tests {
    use super::{DEFAULT_BUDGET, Mode, bind_addr};
    use std::time::Duration;
