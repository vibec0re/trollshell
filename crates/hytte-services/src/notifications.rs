...
///
/// Unreachable by construction, and the three things every transition below
/// preserves: `timeout: None` with an armed `deadline`; `hover_count > 0` with
/// an armed `deadline`; a `deadline` without its `abort` (or vice versa, save
/// for the inherent race where a sleep has fired but its `dismiss` — which
/// drops the whole entry — hasn't run yet).
#[derive(Debug)]
struct TimerState {
    ...
    /// the last hover leaves. Seeded to the resolved timeout so a never-hovered
    /// timer resumes with its full duration; meaningless (and unread) while
    /// `timeout` is `None`, since a sticky state never arms.
    remaining: Duration,
}
...
impl TimerState {
    fn new(timeout: Option<Duration>) -> Self {
        Self {
            ...
            hover_count: 0,
            deadline: None,
            remaining: timeout.unwrap_or_default(),
        }
    }
...
/// entry.
///
/// Called synchronously by the D-Bus `Notify` handler / [`post_local`], so a
/// live notification always has an entry by the time its toast can be mapped and
/// hovered (mapping costs a compositor round-trip; this is a mutex lock on the
/// posting thread) — which is what lets [`pause_expiry`] / [`resume_expiry`] be
/// the sole owners of the hover count instead of sharing that fact with the
/// widget (#619). The pre-#619 split of this into `arm_expiry` + a
/// `disarm_expiry` that *removed* the entry is exactly how the two drifted: a
/// sticky post left nothing for the overlay's hold to land on, so the next
/// finite re-post minted a `hover_count: 0` entry under a hovered card.
///
...
fn set_expiry(id: u32, timeout: Option<Duration>) {
    ...
    if let Ok(mut timers) = shared.timers.lock() {
        ...
        // so a stale sleep can't dismiss the refreshed notification early — or
        // "expire" one that just turned sticky. Unconditional rather than driven
        // off the returned action: it must cover `Nothing` too (a re-post
        // arriving while a hover is held), and aborting an already-cleared
        // handle is free.
...
impl NotificationsIface {
    ...
    async fn notify(
        ...
        // arms the auto-dismiss sleep for a finite timeout, and for a sticky one
        // (`timeout == None`) stands the countdown down — cancelling the stale
        // sleep a `replaces_id` re-post left behind — while keeping the entry, so
        // a hover taken during the sticky phase is still recorded (#619).
...
mod tests {
    ...
    fn timer_arms_full_timeout_when_not_hovered() {
        let mut s = TimerState::new(Some(Duration::from_secs(5)));
        let now = Instant::now();
        assert_eq!(
    ...
    fn clear_timer_removes_only_the_given_id() {
        ...
        map.insert(
            7,
            TimerEntry {
                state: TimerState::new(Some(Duration::from_secs(5))),
                abort: None,
            },
        );
        map.insert(
            8,
            TimerEntry {
                // A sticky entry is a real entry now (#619) and `dismiss` is the
                // only thing that drops one.
                state: TimerState::new(None),
                abort: None,
            },
        );
