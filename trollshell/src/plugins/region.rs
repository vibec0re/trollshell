...
fn build_panel_child(
    ...
    // `overlays::sidebar::close_all` (`overlays/sidebar.rs:921`), both driven
...
mod gtk_tests {
    ...
    /// `card.append(&crate::plugins::sidebar_lead_slot())` (`sidebar.rs:593`), so
    ...
    ///
    /// `close_all` (`overlays/sidebar.rs:893-922`) destroys the toplevel while
    /// `SidebarPanel` still holds its own `revealer` clone (`sidebar.rs:135-150`
    ...
    fn a_destroyed_sidebar_surface_frees_its_plugin_regions() {
        ...
        // `sidebar::install`'s nesting (`sidebar.rs:303-323`), with the region
        // appended into the card as a temporary exactly as `build_card` does
        // (`sidebar.rs:593`). The scroller (and GTK's own `GtkViewport` inside
        ...
        // clones (`sidebar.rs:635-646`), and `close_all` only **aborts** it
        // (`sidebar.rs:903`). Abort is not release — `JoinHandle::abort` is
        ...
        // (`sidebar.rs:135-150`): abort the subscription, destroy the toplevel,
        ...
        assert!(
            ...
            "a hot-unplugged sidebar must free the plugin regions inside its \
             ...
             aborted/cancelled explicitly, `sidebar.rs:903`/`:907`/`:917-919`, \
