    ...
    in
    {
      packages = forAllSystems (
        ...
        let
          ...
          # plus (#614) the hand-written per-plugin env-knob reference
          # (`docs/plugin-env.md`, #573) rendered onto the same docs site so
          # it stops being repo-only. Evaluate BOTH platform modules
          # together: they share the module-common base, deduped by its
          # stable `key`, so the merged option tree is the full
          # cross-platform + NixOS + home-manager surface in one place.
          # `nixosOptionsDoc` renders it to CommonMark; both it and
          # `plugin-env.md` are wrapped into standalone HTML pages via the
          # same `cmark-gfm` call (tiny — no pandoc/Haskell closure). The
          # `-gfm` flavor over plain `cmark` is only so `plugin-env.md`'s
          # pipe tables render as actual `<table>`s — plain CommonMark has no
          # table syntax and would print the `|` characters literally.
          # `_module.check = false` stops the config bodies (which set
          # options owned by nixpkgs/home-manager that we do not declare
          # here) from tripping the unknown-option check; the doc only
          # forces option *metadata*, never the config, so nothing
          ...
          options-doc =
            let
              ...
              # The hand-written doc #614 is publishing. A plain repo-relative
              # path (same style as the `callPackage ./nix/*.nix` calls
              # above) — nix copies the single tracked file into the store,
              # giving a store path just like `doc.optionsCommonMark` above.
            ...
            pkgs.runCommand "trollshell-options-doc"
              ...
              ''
                ...
                # Shared page chrome so every page on the site reads as one
                # document instead of ad hoc ones.
                # `render_page TITLE MD_FILE [H1]` writes the finished HTML to
                # stdout; --extension table is harmless on options.md (which
                # has no pipe tables) so both pages go through the exact same
                # cmark-gfm call. H1 is optional: options.md has no top-level
                # heading of its own (nixosOptionsDoc starts straight at
                # `##`), so it's injected here; plugin-env.md already opens
                # with its own `# …` heading, so it's omitted for that page.
                render_page() {
                  title="$1"
                  md="$2"
                  heading="$3"
                ...
                # The root page (#614): a small landing page linking both
                # documents, rather than a byte-copy of options.html now that
                # there are two of them.
                {
                  echo '<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">'
                  echo '<meta name="viewport" content="width=device-width, initial-scale=1">'
                  echo '<title>trollshell docs</title>'
                  echo '<style>body{max-width:55rem;margin:2rem auto;padding:0 1rem;font-family:system-ui,sans-serif;line-height:1.5}li{margin:.5em 0}</style>'
                  echo '</head><body><h1>trollshell docs</h1><ul>'
