...
trollshell        → the binary; depends on `hytte` — plus `hytte-preem` directly: the shell reads its widgets' **geometry and palette** out of the kit (`LedMatrix`, `Gauge::dial`, `palette_snapshot`, `with_pins`) and hands those to the shaders for the Stats drawer's per-core LED panel. That is the kit, NOT the plugin SDK; the shell still never links `hytte-plugin` — and `hytte-config` directly too: `trollshell/src/config/*` declares the shell's own config-file schemas (`core-leds.toml` the pilot) over `hytte-config`'s layering/writer, adding no new resolved package (already a `hytte-services`/`trollshell-control-center` dependency)
...
hytte-plugin-clock-demo → the reference plugin: pure manifest/init/update/view + one-line main — and, since #1388, the **smallest two-instance demo**: the same binary renders the sidebar card its manifest mounts (`Mount::SidebarTop`; since #1408 a preem clock — a split-flap `HH:MM`, a dot-matrix date line and a seconds sweep, all off the host `Clock` push, the whole card one flat button whose click opens the plugin's own page, which a sidebar mount gets in the centered dialog) or, under a bar-family `HYTTE_PLUGIN_MOUNT`, the `HH:MM` seven-segment chip plus the drawer page its click opens via `Effect::OpenPage(Page::PluginSelf)` — the same page, from one builder. The choice is `Mount::is_bar` over `hytte_plugin::effective_mount` (#1317) and nothing else — no flag, no env knob of its own, no second file — so a bar instance is a second `programs.trollshell.plugins.<id>` entry with `mount = "BarCenter"` pointing at the same package. #1388 folded `hytte-plugin-bar-clock-demo` (#349's bar-chip reference) in here on `hytte-plugin-stats`' shape, since "the plugins can be launched multiple times [so] it makes no sense to have a bar-clock-demo and a clock-demo" (Annika, #1163)
...
- `config/` — the shell's own config-file **schemas** (`core-leds.toml`, `workspaces.toml`) declared over `hytte-config`'s layering/writer: one module per subsystem, each a type + a file name + a documented `DEFAULT_TOML`. `hytte-config` owns the mechanism (search path, merge rules, the reader/writer); this directory owns what is genuinely per-subsystem.
...
- `plugin_launcher.rs` — the declarative plugin **launcher** (#419): reads the nix-written `trollshell/plugins.json` (XDG config, rendered from `programs.trollshell.plugins`) and launches each enabled plugin as a transient `trollshell-plugin-<id>` user unit via `systemd-run --user`, re-reading `plugins.json` and reconciling whenever it changes on disk (#1399 — a content-hash poll over every candidate path, which is what reaches a NixOS-module deploy); the control-center Plugins tab's start/stop (#348) routes through it and, since #1400, its switch persists — kept as a difference from the declared value in `$XDG_STATE_HOME/trollshell/plugins.toml` and folded into each plugin's `enabled` unless nix pins it (`_locked`, by `enable`'s nix priority) — and its `extra_env` spawn hook is where #392's key injection rides. Hand-installed static units keep working (legacy fallback): reconcile never stops a unit it did not stamp, even for a declared-off id — which since #1400 is every bundled id by default (`availablePlugins`) — but it cannot start a static unit whose id is declared, so dropping the id from `availablePlugins` is how one is handed over entirely.
- `revision.rs` — resolves the build's git revision via `TROLLSHELL_REV` (runtime env → compile-time env → `"dev"` fallback), the same three-tier shape as `assets.rs` (#601). The nix side injects it **only** through the cheap wrapper slices' `preFixup` (`nix/package.nix`, `nix/control-center.nix`) — never as a compile-time env, which would rehash the single `workspace` crane compile on every commit. Surfaced over D-Bus as `Control.Revision`, read by `trollshell-control-center`'s footer (#601/#959).
