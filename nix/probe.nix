# Builds the `hytte-ecal` `probe` example binary for the EDS nixosTest
# (checks.eds-nixos-test). Mirrors package.nix's crane setup — the deps build
# still compiles the whole workspace lock, so it needs the same buildInputs +
# bindgen/pipewire handling — but targets the probe example instead of the
# trollshell binary, and skips the test/install chrome.
{
  lib,
  craneLib,
  rustPlatform,
...
let
  ...
  commonArgs = {
    ...
    cargoExtraArgs = "-p hytte-ecal --example probe";
    doCheck = false;
  ...
  # than the `probe` example target, which only exists in the real source used
  # by buildPackage below.
...
craneLib.buildPackage (
  ...
  // {
    ...
    postInstall = ''
      if [ ! -e "$out/bin/probe" ]; then
        bin="$(find target -type f -name probe -path '*examples*' | head -1)"
        install -Dm755 "$bin" "$out/bin/probe"
      fi
    '';
  }
)
