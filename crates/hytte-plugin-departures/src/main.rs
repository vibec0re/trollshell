...
use feed::Row;
use hytte_plugin::proto::{Effect, Manifest, Mount, Node, StateKey};
...
fn lead_label(now_unix: i64, actual_unix: i64, walk_minutes: u32) -> (String, bool) {
    ...
    let slack = (actual_unix - now_unix) - i64::from(walk_minutes) * 60;
...
impl Plugin for Board {
    ...
    fn update(&mut self, input: Input<Self::Msg>) -> Vec<Effect> {
        match input {
            ...
            // host's clock pump), so tolerate `None`.
            ...
            // no `RunCommand`) and `Event` (no interactive nodes until #236 arms
            // rows), plus any future kind. A wildcard keeps new variants from
            // breaking the build.
            _ => {}
        }
        // The board never asks the shell for anything; all its I/O is its own.
        Vec::new()
    }
        ...
        let children = match &self.state {
            ...
            BoardState::Ok { items, .. } | BoardState::Stale { items, .. } => {
                ...
                if items.is_empty() {
                    ...
                    {
                        kids.push(row_node(r, self.now_unix));
                    }
                }
...
/// the `{token} · HH:MM` time cell, and an optional delay badge. Mirrors the
/// native row's structural classes verbatim; the leave-by fade and the
/// cancelled strike ride the same `ts-*` classes.
fn row_node(r: &Row, now_unix: i64) -> Node {
...
mod tests {
    ...
    fn view_data_renders_rows_and_omits_departed() {
        ...
        // First row: badge S9 + destination + time, no delay badge.
        let Node::Row {
            classes, children, ..
        } = &kids[0]
        else {
    ...
    fn view_unreachable_row_is_faded() {
        ...
        let kids = root_children(&board);
        let Node::Row { classes, .. } = &kids[0] else {
            panic!("row is a Row");
        };
    ...
    fn view_stale_shows_rows_then_a_footer() {
        ...
        assert!(matches!(&kids[0], Node::Row { .. }));
        assert!(matches!(
            &kids[1],
            Node::Text { text, classes, .. }
