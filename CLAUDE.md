...
hytte-plugin-clock-demo → the reference plugin: pure manifest/init/update/view + one-line main — and, since #1388, the **smallest two-instance demo**: the same binary renders the sidebar card its manifest mounts (`Mount::SidebarTop`, a label + a button opening the power menu) or, under a bar-family `HYTTE_PLUGIN_MOUNT`, the `HH:MM` seven-segment chip plus the drawer page its click opens via `Effect::OpenPage(Page::PluginSelf)`. The choice is `Mount::is_bar` over `hytte_plugin::effective_mount` (#1317) and nothing else — no flag, no env knob of its own, no second file — so a bar instance is a second `programs.trollshell.plugins.<id>` entry with `mount = "BarCenter"` pointing at the same package. #1388 folded `hytte-plugin-bar-clock-demo` (#349's bar-chip reference) in here on `hytte-plugin-stats`' shape, since "the plugins can be launched multiple times [so] it makes no sense to have a bar-clock-demo and a clock-demo" (Annika, #1163)
