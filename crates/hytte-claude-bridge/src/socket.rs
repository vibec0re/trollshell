...
//! socket, and this module is `trollshell::plugins::listener`'s sequence with
//! the names changed: **lock → probe → unlink a *stale* socket → bind →
//! tighten to `0600`**.
//!
...
pub async fn take_socket(path: &Path) -> std::io::Result<SocketClaim> {
    ...
    // so a failure here unbinds rather than serving an open endpoint.
...
mod tests {
    ...
    async fn a_stale_socket_is_reclaimed() {
        ...
        let dead_inode = std::fs::metadata(&path).expect("exists").ino();
        drop(dead);
        ...
        assert_ne!(
            std::fs::metadata(&path).expect("exists").ino(),
            dead_inode,
            "the stale inode was replaced by this process's own",
        );
    ...
    /// incumbent's inode exactly where it is and refuses to serve, rather than
    /// unlinking a working endpoint and logging success onto a socket nobody
    /// can reach (#995's failure, and #436's before it).
    ///
    /// Mutation: delete the `socket_in_use` probe from `take_socket`. Then the
    /// live socket is unlinked and rebound, the inode changes, and this is the
    /// test that goes red — the other socket tests all stay green, because a
    /// stale socket cannot tell the two behaviours apart.
    #[tokio::test]
    async fn a_live_socket_is_never_unlinked() {
        ...
        let incumbent_inode = std::fs::metadata(&path).expect("exists").ino();
        ...
        assert_eq!(
            std::fs::metadata(&path).expect("still exists").ino(),
            incumbent_inode,
            "the refused instance never unlinked or rebound the live socket",
        );
        // …and the incumbent is still serving, which is the whole point.
        let (client, accepted) =
    ...
    async fn a_second_start_stands_down_on_the_lock() {
        ...
        let inode = std::fs::metadata(&path).expect("exists").ino();
        ...
        assert_eq!(
            std::fs::metadata(&path).expect("exists").ino(),
            inode,
            "and must not have touched the first one's socket",
        );
        drop(first);
    }
