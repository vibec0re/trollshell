      ...
      type = lib.types.submodule {
        options = {
          ...
          window = lib.mkOption {
            type = lib.types.submodule {
              options = {
                workspace = lib.mkOption {
                  # Checked at eval like `socket` above, and unlike it this
                  # is NOT a whole-file rule: `WindowConfig::workspace` is
                  # the one key in `agents.toml` with a per-key verdict
                  # (#1306), so a bad value costs itself and nothing else at
                  # load time. The check is here anyway for the other half
                  # of MEDIUM-2's argument — nix can refuse the value in
                  # front of the operator who typed it, naming the option
                  # path, instead of leaving them a journal line they will
                  # not read and a workspace focus that quietly went
                  # somewhere else.
                  #
                  # STRICTER than the Rust rule, the safe direction for a
                  # mirror (`socket`'s comment above): Rust trims first, so
                  # `"  hive  "` is legal there and refused here; every
                  # value this option accepts loads unchanged. What both
                  # sides refuse is the shape that would break the argv —
                  # blank, a leading `-` (niri's command line is `clap`, so
                  # it would be read as a flag), or a control character. An
                  # inner space is fine on both sides: the name travels as
                  # its own argv element and niri's kdl quotes it.
                  # Pinned by `flake.nix`'s
                  # `checks.nixos-module-agents-bad-workspace`.
                  type = lib.types.nullOr (
                    lib.types.strMatching "[^-[:space:][:cntrl:]]([^[:cntrl:]]*[^[:space:][:cntrl:]])?"
                  );
                  default = null;
                  example = "hive";
                  description = ''
                    Which niri workspace the agents' companion windows
                    belong on (#1306). The card's "Open terminals" button
                    focuses this workspace FIRST and then launches one
                    window per running agent, so they tile there as columns
                    instead of landing wherever the compositor happens to
                    put them.

                    It is a **name**, not an index, and it has to match the
                    niri side: pair it with `workspace "<name>" { }` and a
                    `window-rule` sending
                    `mov.vibec0re.trollshell.AgentWindow.*` to
                    `open-on-workspace "<name>"` — `etc/niri/agent-windows.kdl`
                    ships both. Nothing cross-checks the two spellings, so a
                    mismatch focuses an empty workspace and opens the windows
                    on another one.

                    Focusing needs `niri` on the plugin's `PATH`; without it
                    the windows still open, just not gathered. `null` (the
                    default) sets no `workspace` key at all, leaving the
                    overlay or the plugin's built-in default
                    (`config.rs`'s `DEFAULT_WORKSPACE`, `hive`) to decide.
                  '';
                };
              };
            };
            ...
            description = ''
              The `[window]` table of `agents.toml` — where the per-agent
              companion windows go. Every field defaults to `null` ("no
              opinion") and the platform modules' `prune` drops the emptied
              table, so leaving this unset renders no `[window]` heading at
              all (pinned by `checks.modules-agents-absent-when-unset`).
            '';
          };
